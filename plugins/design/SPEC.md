...
approved: 2026-09-05
amended: 2026-09-06 — `api --records` declares an endpoint no screen action produced. The generator builds one per write action, `integrations[]` already let the owner declare the outside world, and between them sat a hole: a sign-in writes nothing a screen declares, so no command in design could ever produce its route — CR-005 asked for `POST /api/auth/login` and there was nowhere for the answer to land. An `apis[]` entry whose action has no generated endpoint now mints one at `reviewed`, and is refused without both `method` and `path`, or when the screen declares no such action. A route nobody typed is a route somebody guessed.
