...
amended: 2026-09-06 — the callsheet row a finding routed to design gets now depends on where its change request actually is. The rule filtered `d.status !== "verified" && d.cr` and always printed `/change:impact`, so DEF-rental-003 kept asking for the impact of CR-004 long after CR-004 was walked, applied and closed. Three states, three answers: open and not yet walked → `/change:impact`; open and walked → nothing from qa, because apply and close are change's own rules; closed → `/qa:run <TC>`, since a change that answered a finding on paper is still a claim until something green says so (P6). A `cr` naming no record on disk gets a row of its own instead of a silent row about nothing.
